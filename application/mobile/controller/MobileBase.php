        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            if (empty($_SESSION['openid'])) {
                ...
                if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
					...
					if($_GET['sarkcode']){	//如果存在美人柜编码
						$wxuser['sarkcode'] = $_GET['sarkcode'];
                    }
                    ...
                    if($data['status'] == 1){
                        ...
                        $oauth_user = M('OauthUsers')
						->where(array('openid'=>$data['result']['openid'] , 'oauth'=>'weixin'))
						->find();
                        $oauth_user && $data['result']['openid'] = $oauth_user['openid'];
                        //session('user',$data['result']);
						$_SESSION['openid'] = $oauth_user['openid'];
						$_SESSION['wxuser'] = $wxuser;
						$_SESSION['third_oauth'] = $oauth_user;                                       
                    }
					
					if(!$oauth_user['is_bind']){
                        //用户未关联账号, 跳到关联账号页
						$_SESSION['user_id'] = 0;
						setcookie('user_id',0,null,'/');
						setcookie('uname',$wxuser['nickname'],null,'/');
                        return $this->redirect(U('Mobile/User/bind_guide'));
                    }else{
						$user = M('Users')->where('user_id',$oauth_user['user_id'])->find();
						//session('user',$user);
						$_SESSION['user'] = $user;
						//setcookie('user_id',$oauth_user['user_id'],null,'/');
						//setcookie('uname',$user['nickname'],null,'/');
						$_COOKIE['uname'] = $user['nickname'];
					}
                }
            }else{
			
				$user_id = M('OauthUsers')
						->where(array('openid'=>$_SESSION['openid'] , 'oauth'=>'weixin'))
						->GetField('user_id');
				if($user_id > 0){
