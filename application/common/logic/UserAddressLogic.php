...
class UserAddressLogic extends model
{
     ...
     * @return array
     */
    public function getUserAddress_default($user_id)
    {
        $where = array(
            'ua.user_id' => $user_id,
            'ua.is_default' => 1
        );
        $user_default = M('user_address')
            ->alias('ua')
            ->field('ua.*,r1.name AS province_name,r2.name AS city_name,r3.name AS district_name,r4.name AS twon_name')
            ->join('__REGION__ r1','r1.id = ua.province','LEFT')
            ->join('__REGION__ r2','r2.id = ua.city','LEFT')
            ->join('__REGION__ r3', 'r3.id = ua.district','LEFT')
            ->join('__REGION__ r4','r4.id = ua.twon','LEFT')
            ->where($where)
            ->find();
        return $user_default;
    }
    ...
    public function getAddressDetail($address_id){
        $res = M('user_address')->alias('ua')
            ->field('ua.address,ua.province,ua.city,ua.district,r1.name AS province_name,r2.name AS city_name,r3.name AS district_name,r4.name AS town_name')
            ->join('__REGION__ r1','r1.id = ua.province','LEFT')
            ->join('__REGION__ r2','r2.id = ua.city','LEFT')
            ->join('__REGION__ r3', 'r3.id = ua.district','LEFT')
            ->join('__REGION__ r4','r4.id = ua.twon','LEFT')
