...
class ActivityLogic extends Model
{
    ...
    /**
     * 优惠券列表
     * @param type $atype 排序类型 1:默认id排序，2:即将过期，3:面值最大，4:未开始，5:可领取，6:已过期 
     * @param $user_id  用户ID
     * @param type $p 第几页
     */
    public function getCouponList($atype, $user_id, $p = 1)
    {
        ...
        if ($atype == 2) {
            ...
            //未开始
            $order = ['add_time' => 'desc'];
            $where['send_start_time-UNIX_TIMESTAMP()'] = ['gt', 0];
        } elseif ($atype == 5) {
            //可领取
            $order = ['add_time' => 'desc'];
            $where['send_start_time-UNIX_TIMESTAMP()'] = ['elt', 0];
            $where['send_end_time-UNIX_TIMESTAMP()'] = ['egt', 0];
        } elseif ($atype == 6) {
            //已过期
            $order = ['add_time' => 'desc'];
            $where['use_end_time-UNIX_TIMESTAMP()'] = ['lt', 0];
        }
...
//                        $coupon_list[$k]['isget'] = 1;  //标识为已领取
    ...
    public function getOrderPromSimpleInfo($user, $goods)
    {
        ...
        $cur_time = time();
        $sql = "select * from __PREFIX__prom_order where start_time <= $cur_time "
                . "AND end_time > $cur_time AND FIND_IN_SET({$user['level']}, `group`)";
        $data = [];
        $po = Db::query($sql);
        if (!empty($po)) {
            foreach ($po as $p) {
     ...
     * @return type
     */
    public function getOrderPayProm($user)
    {
        if (!isset($user['level'])) {
            return [];
        }
