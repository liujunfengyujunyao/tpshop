...
class User extends Base {
    ...
    public function index(){
        //工厂店列表
        $Stores = M('store')->field('store_id,user_id,store_name')->select();
    ...
    public function ajaxindex(){
        ...
        I('first_leader') ? $condition['first_leader'] = I('first_leader') : false;
        
        //增加搜索条件,只显示非特殊会员组会员
//        $level_list = M('user_level')->field('level_id')->where('is_special = 0')->select();
//        $val = arr_to_str($level_list);
//        $where['level'] =array('IN',$val);


        //屏蔽一二三级下线人查看   @author: Ly  @date:2017-7-28
        ...
        $model = M('users');
        $count = $model->where($condition)->where($where)->count();
        ...
        $userList = $model->where($condition)->where($where)->order($sort_order)->limit($Page->firstRow.','.$Page->listRows)->select();
       ...
       /* $this->assign('first_leader',$first_leader);
        ...
        $this->assign('store',M('store')->getField('user_id,store_name'));
    ...
    public function detail(){
        $uid = I('get.id');
        $user = D('users')->alias('u')->join('user_level l', 'l.level_id = u.level', 'left')->where(array('u.user_id'=>$uid))->find();
        $level_list = M('user_level')->where('is_special=0')->order('level_id')->select();
        if(!$user)
        ...
        //会员详细信息屏蔽一二三下线 @author: Ly @date:2017-7-28
    ...
    public function signList() {
        $count = Db::name('User_sign')->count();
        $Page  = new Page($count, 20);
        $list = Db::name('User_sign')->alias('s')->field('u.nickname, s.*')->join('users u', 'u.user_id = s.user_id', 'left')->limit($Page->firstRow.','.$Page->listRows)->select();
        $show  = $Page->show();
        $this->assign('page', $show);
        $this->assign('list', $list);
        return $this->fetch();
