...
class Goods extends Base {
    ...
    public function addEditGoods()
    {
        ...
        //赠品列表
        $send_type = 8;   //上线需更改
        $send_goods = M('goods')->field('goods_id,goods_name')->where('cat_id',$send_type)->select(); //赠品列表
        $goods_shipping_area_ids = explode(',', $goodsInfo['shipping_area_ids']);
        $this->assign('send_goods',$send_goods);
    ...
    public function delGoods()
    {
        ...
        if($error)
        {
            $this->success($msg,U('Admin/Goods/goodsList'));
        }
