...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $count = M('order')->alias('o')->where($condition)->count();
    ...
    */
    ...
    public function refund_order(){
        ...
        if($data['pay_status'] == 3 && $data['refund_type']== 1){
            //修改退款金额为用户实际付款金额   $author: Ly @date:2017-12-1
        ...
        if($data['pay_status'] == 3 && $data['refund_type']== 0){
            ...
            if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
                ...
                if($order['pay_code'] == 'weixin'){
                    ...
                    $payment = array('transaction_id'=>$order['transaction_id'],'total_fee'=>$order['order_amount'],'refund_fee'=>$return_money);
                    $data['transaction_id'] = $payment['transaction_id'];
                    $data['total_fee'] = $payment['total_fee'];
                    $data['refund_fee'] = $payment['refund_fee'];
                    $result = $payment_obj->payment_refund($payment);
                    if($result['return_code'] == 'SUCCESS'){
                        if ($result['result_code'] == 'FAIL') {
                            ...
                            //合伙人、工厂店分佣金额需追回 by Dh 2017-12-8
                            $rebate = DB::name('Rebate_log')->where(array('order_sn'=>$order['order_sn']))->find();
                            if ($rebate) {
                                if ($rebate['status'] == 3) {
                                    $order_rate = tpCache('distribut.order_rate'); // 订单分成比例
                                    //$commission = $return_goods['refund_deposit'] * ($order_rate / 100); // 要退余额 乘以 订单分成比例
                                    $commission = $order['amount'] * ($order_rate / 100);

                                    $first_rate = tpCache('distribut.first_rate'); // 一级上线获佣比例
                                    $second_rate = tpCache('distribut.second_rate'); // 二级上线获佣比例

                                    $first_money = $commission * ($first_rate / 100); // 一级需追回的佣金
                                    $second_money = $commission * ($second_rate / 100); // 二级需追回的佣金

                                    $desc = '订单已申请售后，佣金追回';

                                    $first = DB::name('Rebate_log')->where(array('order_sn'=>$order['order_sn'], 'level'=>1))->getField('user_id'); //一级上线id
                                    $second = DB::name('Rebate_log')->where(array('order_sn'=>$order['order_sn'], 'level'=>2))->getField('user_id'); //二级上线id
                                    accountLog($first, -$first_money, 0, $desc, 0, $order['order_id'], $order['order_sn']);
                                    accountLog($second, -$second_money, 0, $desc, 0, $order['order_id'], $order['order_sn']);
                                    $data = array('status'=>4, 'remark'=>$desc);
                                    DB::name('Rebate_log')->where(array('order_sn'=>$order['order_sn']))->data($data)->update();
                                } else {
                                    $data = array('status'=>4, 'remark'=>'订单已申请售后');
                                    DB::name('Rebate_log')->where(array('order_sn'=>$order['order_sn']))->data($data)->update();
                                }
                            }
                            M('order')->where(array('order_id'=>$order['order_id']))->save($data);
    ...
    public function detail($order_id){
        ...
        if($has_user){
            $user = M("users")->alias('u')->field('u.nickname')->join('__ORDER__ o', 'o.user_id = u.user_id')->where("o.order_id=$order_id")->find();
        }
    ...
    /**
     * 发货单列表
     * update Dh 2017-8-11 添加合伙人订单的选择
     */
    public function delivery_list(){
        $group_list = [
            'order_list' => '普通订单',
            'partner_order_list' => '配货单(－>合伙人)'
        ];
        $this->assign('group_list', $group_list);
        $inc_type = I('get.inc_type', 'order_list');
        $this->assign('inc_type', $inc_type);
        if($inc_type == 'order_list'){
            return $this->fetch();
        }else {
    ...
    public function return_info()
    {
        ...
        if(IS_POST)
        {
            ...
            $orderLogic->orderActionLog($return_goods['order_id'],'refund',$note);
            $this->success('修改成功!');
            exit;
        }
            ...
            if($order['pay_code'] == 'weixin'){
                ...
                if($result['return_code'] == 'SUCCESS'){
                    if ($result['result_code'] == 'FAIL') {
                        ...
                        //合伙人、工厂店分佣金额需追回 by Dh 2017-12-8
                        $rebate = DB::name('Rebate_log')->where(array('order_sn'=>$return_goods['order_sn']))->find();
						
                        if ($rebate) {
                            if ($rebate['status'] == 3) {
                                $order_rate = tpCache('distribut.order_rate'); // 订单分成比例
                                $commission = $return_goods['refund_money'] * ($order_rate / 100); // 要退余额 乘以 订单分成比例
                                ...
                                $first = DB::name('Rebate_log')->where(array('order_sn'=>$return_goods['order_sn'], 'level'=>1))->getField('user_id'); //一级上线id
								$second = DB::name('Rebate_log')->where(array('order_sn'=>$return_goods['order_sn'], 'level'=>2))->getField('user_id'); //二级上线id
                                accountLog($first, -$first_money, 0, $desc, 0, $return_goods['order_id'], $return_goods['order_sn']);
                                accountLog($second, -$second_money, 0, $desc, 0, $return_goods['order_id'], $return_goods['order_sn']);
                                $data = array('status'=>4, 'remark'=>$desc);
                                DB::name('Rebate_log')->where(array('order_sn'=>$return_goods['order_sn']))->data($data)->update();
                            } else {
                                $data = array('status'=>4, 'remark'=>'订单已申请售后');
                                DB::name('Rebate_log')->where(array('order_sn'=>$return_goods['order_sn']))->data($data)->update();
                            }
    ...
    public function add_return_goods()
   {
    ...
    public function add_order()
    {
        ...
        //  获取支付方式
        $payment_list = M('plugin')->where(array('status'=>1,'type'=>'payment'))->select();
        //  获取工厂店   @author: Ly @date: 2017.8.8
        $store_list = M('store')->field('store_id,store_name')->select();
        //  获取自提点   @author: Ly @date:2017.8.8
        $pickup_list = M('pick_up')->field('pickup_id,pickup_name')->select();
        if(IS_POST)
        {
            ...
            //  获取选定的配送方式,根据配送方式处理订单数据  @author: Ly @date:2017.8.9
            $order['shop_way'] = I('shop_way');
            if($order['shop_way'] == 1){
                $order['shipping_code'] = I('shipping');// 物流方式
                $order['shipping_name'] = M('plugin')->where(array('status'=>1,'type'=>'shipping','code'=>I('shipping')))->getField('name');
            }elseif($order['shop_way'] == 2){
                $order['store_id'] = I('store_id');
                $order['shipping_code'] = 'store';
                $order['shipping_name'] = M('store')->where(array('store_id'=> I('store_id')))->getField('store_name');
            }elseif($order['shop_way'] == 3){
                $order['store_id'] = M('pick_up')->where(array('pickup_id' => I('pickup_id')))->getField('store_id');
                $order['shipping_code'] = 'ziti';
                $order['shipping_name'] = M('pick_up')->where(array('pickup_id' => I('pickup_id')))->getField('pickup_name');
    ...
    public function invoice($data) {
        ...
        if (!empty($data['mobile'])) {
            $where['d.phone'] = array('like', '%' . $data['mobile'] . '%');
        }
        ...
        $list = DB::name('delivery')
                ->alias('d')
                ->field("d.id, d.name, d.phone, r1.name as province_name, r2.name as city_name, r3.name as town_name, d.addtime, FROM_UNIXTIME(d.confirm_time, '%Y-%m-%d %H:%i:%s') as confirm_time, d.express_name, d.express_code, a.user_name")
    ...
    public function invoice_info() {
        ...
        $info = DB::name('delivery')
                ->alias('d')
                ->field("d.name, d.phone, r1.name as province_name, r2.name as city_name, r3.name as town_name, d.express_name, d.express_code, d.addtime, FROM_UNIXTIME(d.confirm_time, '%Y-%m-%d %H:%i:%s') as confirm_time, a.user_name")
                ->join('__REGION__ r1', 'r1.id = d.province_id', 'LEFT')
                ->join('__REGION__ r2', 'r2.id = d.city_id', 'LEFT')
                ->join('__REGION__ r3', 'r3.id = d.town_id', 'LEFT')
                ->join('__ADMIN__ a', 'a.admin_id = d.edituser', 'LEFT')
        ...
        return $this->fetch('partner_order_info');
    }
    ...
    public function invoice_print(){
        ...
        $this->assign('info', $info);
        $this->assign('orderGoods', $goods);
        $template = I('template','invoice_print');
        return $this->fetch($template);
    }
    ...
    public function account_edit(){
        ...
        $refund_deposit = trim(I('refund_deposit/f',0)); //要退余额
        $refund_integral = trim(I('refund_integral/d',0)); //要退积分
        $return_info = M('return_goods')->where(['user_id'=>$user_id,'id'=>$return_id])->find();
        if(empty($return_info))
            $this->ajaxReturn(['status'=>0,'msg'=>"参数有误"]);
        $return_info['desc'] = "订单退货退款";
        $orderLogic = new OrderLogic();
        $res = $orderLogic->alterReturnGoodsStatus($return_id, $return_info['order_id']);
        if($res){
            $orderLogic->closeOrderByReturn($return_info['order_id']);
        }else{
            $this->ajaxReturn(['status'=>-1,'msg'=>"订单操作失败"]);
        }
        if(accountLog($user_id,$refund_deposit,$refund_integral,$return_info['desc'],0,$return_info['order_id'],$return_info['order_sn']))
        {
            $rebate = DB::name('Rebate_log')->where(array('order_sn'=>$return_info['order_sn']))->find();
            if ($rebate) {
                if ($rebate['status'] == 3) {
                    //合伙人、工厂店分佣金额需追回 by Dh 2017-12-8
                    $order_rate = tpCache('distribut.order_rate'); // 订单分成比例
                    $commission = $refund_deposit * ($order_rate / 100); // 要退余额 乘以 订单分成比例
                    ...
                    $first = DB::name('Rebate_log')->where(array('order_sn'=>$return_info['order_sn'], 'level'=>1))->getField('user_id'); //一级上线id
                    $second = DB::name('Rebate_log')->where(array('order_sn'=>$return_info['order_sn'], 'level'=>2))->getField('user_id'); //二级上线id
                    accountLog($first, -$first_money, 0, $desc, 0, $return_info['order_id'], $return_info['order_sn']);
                    accountLog($second, -$second_money, 0, $desc, 0, $return_info['order_id'], $return_info['order_sn']);
                    $data = array('status'=>4, 'remark'=>$desc);
                    DB::name('Rebate_log')->where(array('order_sn'=>$return_info['order_sn']))->data($data)->update();
                } else {
                    $data = array('status'=>4, 'remark'=>'订单已申请售后');
                    DB::name('Rebate_log')->where(array('order_sn'=>$return_info['order_sn']))->data($data)->update();
                }
