...
class Pickup extends Base {
	...
	public function ajaxPickupList(){
		...
		$pickup_where['st.status'] = 1;
		$count = DB::name('pick_up')->alias('p')->join('__STORE__ st','st.store_id = p.store_id','LEFT')->where($pickup_where)->count();
		$Page  = new AjaxPage($count,10);
		$show = $Page->show();

		$pickupList = DB::name('pick_up')
				->alias('p')
				->field('p.*,r1.name as province_name,r2.name as city_name,r3.name as district_name,s.suppliers_name,st.store_name')
                ...
                ->join('__STORE__ st','st.store_id = p.store_id','LEFT')
				->where($pickup_where)
	...
	public function add()
	{
		...
		//获取未绑定自提点工厂店 @author: Ly @date: 2017-8-9   @modify:2017-8-14
        $store_exit = M('pick_up')->group('store_id')->getfield('store_id',true);
        $stores = implode(',',$store_exit);
        $where['store_id'] =array('not in',$stores);
        $store_list = M('store')->field('store_id,store_name')->where($where)->order('store_id ASC')->select();
    ...
    */
	public function edit_address(){
        ...
        //获取工厂店 @author: Ly @date: 2017-8-9
        $store_list = M('store')->field('store_id,store_name')->order('store_id ASC')->select();
