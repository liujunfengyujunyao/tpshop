...
class Admin extends Base {
    ...
    public function log(){
        ...
        //分类列表
        $menulist = getAllMenu();
        foreach($menulist as $k => $rs){
            $k = strtolower($k);
            $name = $rs['name'];
            $temp[$k] = $name;
            $res[] = array('controller' => $k,'name' => $name);
        }
        $p = I('p/d',1);
        $where = array();
        if (IS_POST){
        ...
        //优化原来查询方式,增加条件筛选   @author: Ly @date: 2017-7-29
        $count = DB::name('admin_log')->where($where)->count();
        $Page = new Page($count,20);
        $show = $Page->show();
        $logs = DB::name('admin_log')->alias('l')->join('__ADMIN__ a','a.admin_id =l.admin_id')->where($where)->order('log_time DESC')->limit($Page->firstRow. ',' .$Page->listRows)->select();
