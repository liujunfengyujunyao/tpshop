...
class OrderLogic extends Model
{
     ...
     * @modify Ly   增加核销码以及工厂店
     * @date: 2017-08-04
     */
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $field = 'o.order_id,o.order_sn,o.order_status,o.shipping_status,o.pay_status,o.consignee,o.mobile,o.pay_name,o.goods_price,o.order_amount,o.total_amount,o.add_time,sc.safecode,s.store_name';
        $join = array(['tfs_safecode sc','o.order_sn=sc.order_sn','LEFT'],['tfs_store s','o.store_id=s.store_id','LEFT']);
        $res = DB::name('order')->alias('o')->field($field)->join($join)->where($condition)->limit("$start,$page_size")->order($order)->select();
        ...
        if ($order['shop_way'] == 1) { //合作快递
            ...
            if($res && $res['status'] ==1){
                ...
                $store = Db::name('pick_up')->alias('p')
                        ...
                        ->find();
                $safecode = Db::name('safecode')->where('order_sn', $data['order_sn'])->getField('safecode');
                if($users && $store){
                    $nickname = $users[$user_id]['nickname'];
                    $sender = $users[$user_id]['mobile'];
