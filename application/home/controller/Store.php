...
class Store extends Base
{
    ...
    public function order_list(){
        ...
        $moneys = M('rebate_log')
                ->where(array('user_id'=>$user_id, 'level'=>1, 'status'=>3))
                ->sum('money');
    ...
    public function apply_info() {
        ...
        $info = DB::name('goods_apply')
                ->alias('ga')
                ->field('ga.id, ga.remark, ga.addtime, ga.status, ga.remark, d.express_name, d.express_code, d.addtime as delivery_time, d.confirm_time, ga.delivery_id, ga.edittime')
                ->join('__DELIVERY__ d', 'd.id = ga.delivery_id', 'LEFT')
                ->where("ga.id=$id")
                ->find();
        ...
        $goods = Db::name('goods_apply_info')
                 ->alias('i')
                 ->field('g.goods_id, g.goods_name, g.goods_sn, i.goods_num, g.shop_price')
    ...
    public function rebate_log()
    {
        ...
        $lists = Db::name('rebate_log')->where($where)->order('confirm_time desc')->limit("{$page->firstRow},{$page->listRows}")->select(); //查询日志
