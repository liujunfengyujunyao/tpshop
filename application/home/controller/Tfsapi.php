...
{   
        ...
        if(IS_POST){
            ...
            switch($act){
                case 'get_qrcode':
                    ...
                    break;
                case 'get_history':
                    $openid = $_POST['openid'] ? $_POST['openid'] : '';
                    $this -> get_history($openid);
                    break;
                case 'get_history_':
                    $openid = $_POST['openid'] ? $_POST['openid'] : '';
                    $this -> get_history_($openid);
                    break;
                default:
    ...
    private function get_history($openid){
    ...
    private function get_history_($openid){
        $url = 'http://54.64.191.2/EZM_Work/web_api/GetHistory.php';
        $data = ['loginid' => $openid];
        //定义返回值接收变量；
        $return = httpRequest($url,'POST',$data);
        $return = json_decode($return,1);
