...
class User extends Base{
    ...
    public function index(){
        ...
        $map['lastsign'] = strtotime(date('Y-m-d', time()));
        $check = M('user_sign')->where($map)->find();
        $issign = $check ? 1 : 0;
    ...
    public function account(){
        ...
        //获取账户资金记录
        $logic = new UsersLogic();
        $data = $logic->get_points_log($this->user_id);
    ...
    public function coupon(){
        ...
        foreach ($coupon_list as $k=>&$v) {
            if($v['order_id'] == 0 && $v['use_end_time'] > time()&& $v['status'] == 0){
                //未使用
                $v['pred'] = 3;
            }elseif($v['order_id'] > 0 && $v['use_time'] > 0){
                //已使用
                $v['pred'] = 1;
            }elseif($v['use_end_time'] <= time()){
                //已过期
                $v['pred'] = 2;
            }
            //$v['name'] = substr(strstr($v['name'], '元'), strlen('元'));
        }
    ...
    public function do_login(){
    	...
    	if($res['status'] == 1){
			...
			$nickname = urlencode($res['result']['nickname']);
            ...
            if(check_mobile($username)){
                if($reg_sms_enable){   //是否开启注册验证码机制
                    ...
                    if($check_code['status'] != 1){
                        $data = array('status'=>-1,'msg'=>$check_code['msg']);
...
//            $cartLogic->login_cart_handle($this->session_id,$data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    * 手机验证第1步，检验原手机号
    */
    public function mobile_validate_1()
    {
        setcookie("mobile_key", "", time()-3000);//初始化手机验证状态
        $userLogic = new UsersLogic();
        $user_info = $userLogic->get_info($this->user_id); //获取用户信息
        $user_info = $user_info['result'];
        $config = tpCache('sms');
        ...
        if (IS_POST) {
            ...
            if ($res['status'] == 1) {
                switch ($type) {
                    case 'mobile':
                        $res['url'] = U("Home/User/mobile_validate_2");
                        break;
                    case 'pwd':
                        $res['url'] = U("Home/User/update_pwd_2");
                        break;
                    default:
                        $res['url'] = U("Home/User/mobile_validate_1");
                        break;
                }
                setcookie("mobile_key", "1", time()+3000);
            }
            exit(json_encode($res));
        }else{
    ...
    * 手机验证第2步，检验新手机号
    */
    public function mobile_validate_2()
    {
        if(!isset($_COOKIE['mobile_key']) || ($_COOKIE['mobile_key'] != 1)){//检验是否通过验证
            $this->redirect('Home/User/mobile_validate_1');
        }
        ...
        if (IS_POST) {
            ...
            $logic = new UsersLogic();
            $res = $logic->check_validate_code($code, $new_mobile, 'phone', $session_id, $scene);
            ...
            if ($res['status'] == 1) {
                ...
                if (!$userLogic->update_email_mobile($new_mobile, $this->user_id, 2)){
                    $res = array('status'=>2,'msg'=>'手机号已存在');
                    exit(json_encode($res));
                }else{
                    setcookie("mobile_key", "", time()-3000);//初始化手机验证状态
                    $res['url'] = U("Home/User/mobile_validate_3");
                    exit(json_encode($res));
                }                
            } else {
                $this->error($res['msg']);
    ...
    /*完成手机号更换*/
    public function mobile_validate_3()
    {
        return $this->fetch();
    }
    ...
    public function password()
    {
        ...
        if($user['mobile'] == ''){
            ...
            $this->redirect('Home/User/mobile_validate_1',array('type'=>'pwd'));
        }
    ...
    public function update_pwd_2()
    {
        ...
        if(IS_POST){
            ...
            $userLogic = new UsersLogic();
            $res = $userLogic->password($this->user_id,I('post.new_password'),I('post.confirm_password')); // 获取用户信息
    ...
    {
        $oauth = I('oauth'); 
        $row = M('OauthUsers')->where(array('user_id' => $this->user_id , 'oauth'=>$oauth))->save(array('user_id'=>NULL, 'is_bind'=>0, 'bind_time'=>NULL));
        if ($row) {
    ...
    public function safety_settings()
    {
        ...
        $user_info['email'] = substr($email[0], 0, 3) . '***' . $email[1];
        $this->assign('user',$user_info);
        return $this->fetch();
    }
   ...
   public function message_notice()
   {
        ...
        $this->assign('list', $user_sys_message['message']);
        $this->assign('page', $user_sys_message['page']);
        return $this->fetch();
   }

   /**
    * message_info tfs模板 专用
    * @author ShenCheng
    * @update 2018/03/06 
    */
   ...
   {
        $id = I('id',0);
        $info = M('message')->where("message_id=$id")->find();
     ...
     * @time 2016/09/01
     */
    public function message_notice_1()
    {
     ...
     * @time 2016/09/01
     */
    public function ajax_message_notice_1()
    {
        $type = I('type',0);
        $user_logic = new UsersLogic();
        $message_model = new MessageLogic();
     ...
     * ajax用户消息通知请求
     */
    public function set_message_notice_1()
    {
        $type = I('type');
        $msg_id = I('msg_id');
        $user_logic = new UsersLogic();
    ...
    public function visit_log()
    {
        ...
        $Page = new Page($count, 10);
        $visit_list = M('goods_visit a')->field("a.*,g.goods_name,g.shop_price,g.goods_sn")
        ...
        $this->assign('visit_list', $visit_list); //浏览记录
        return $this->fetch();
    }
...
//                        //不是连续签
//                        if ($daya != $dayb) {
//                            $update_data['signcount'] = 1;                                               //连续签到天数
//                        } else {
//                            if (($signcount+1) > $config['sign_count']) {
//                                $update_data['signcount'] = ['exp', 1];
//                            } else {
