...
function get_user_info($user_id_or_name,$type = 0,$oauth='')
{
	...
	$user = M('OauthUsers')->where([$column=>$user_id_or_name, 'oauth'=>$oauth])->find();
...
function update_user_level($user_id){
    $level_info = M('user_level')->where('is_special=0')->order('level_id')->select();
...
function minus_stock($order){
    ...
    foreach($orderGoodsArr as $key => $val)
    {
        ...
        {   // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
            ...
            $data = array('goods_id'=>$val['goods_id'], 'goods_name'=>$val['goods_name'], 'order_sn'=>$order['order_sn'], 'muid'=>1, 'stock'=>'-'.$val['goods_num'], 'ctime'=>time());
...
//            $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
        ...
        if($order['pay_status'] == 1 && ($order['order_status'] == 1 || $order['order_status'] == 6)  && $order['shipping_status'] == 1) //待收货
        {
            $btn_arr['receive_btn'] = 1;  // 确认收货
//            $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
        }
...
//        $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
    ...
    if($order['order_status'] == 3 && ($order['pay_status'] == 1 || $order['pay_status'] == 3 || $order['pay_status'] == 4)){
...
function update_pay_status($order_sn,$ext=array())
{
    if(stripos($order_sn,'recharge') !== false){
        ...
        // 减少对应商品的库存    @modify:Ly  @date: 2017-8-25   工厂店模式在工厂店出货是减自己对应库存,其他减总库
        if($order['shop_way']==1){
            minus_stock($order);
        }
        /**
         ...
         */
        if($order['shop_way'] == 3){
            $safe['safecode'] = Random();  //生成6位核销码
            $safe['order_sn'] = $order['order_sn']; //订单编号
            $safe['store_id'] = $order['store_id'];  //分派工厂店
            $safe['addtime'] = time();
            M('safecode')->insertGetId($safe);
        }
        ...
        //订单支付完成进行分成记录  @modify: Ly @date: 2017-8-26
        //如果公司直接发快递则不分成 @author: Ly @date: 2017-8-23
        if(($order['shop_way'] != 1) && ($ext['act'] != 'integral')) {
            //查询订单是否使用体验券 by DH 2017-11-21
            $where['c1.order_id'] = $order['order_id'];
            $where['c2.coupon_type'] = 2; //优惠券类型为体验券
            $isset = Db::name('coupon_list')->alias('c1')->join('coupon c2', 'c1.cid=c2.id', 'left')->where($where)->find();

            //分销开关全局
            $distribut_switch = tpCache('distribut.switch');
            if ($distribut_switch == 1 && file_exists(APP_PATH . 'common/logic/DistributLogic.php') && empty($isset)) {
        ...
        $params = array('order_sn'=>$order_sn, 'datetime'=>date("Y-m-d H:i:s",time()));
...
function order_give($order)
{
    ...
    /* 非推荐店服务,赠送推荐店积分 */
    if(tpCache('basic.tuijian')){
        $points_tuijian = M('order_goods')->where("order_id", $order['order_id'])->sum("invite_give_integral * goods_num");

        //获取推荐店负责人ID
        $user_tuijian = M('users')->alias('u')->join('store s', 's.user_id = u.first_leader', 'left')->where(array('u.user_id'=>$order['user_id'], 's.status'=>1))->getField('first_leader');
        $points_tuijian && accountLog($user_tuijian, 0, $points_tuijian, "推荐奖励积分", 0, $order['order_id'], $order['order_sn']);
    }
 ...
 * @param type $shop_way int 配送方式   $modify: Ly     @date:2017-8-26
 */

function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '', $shop_way = 0)
{
