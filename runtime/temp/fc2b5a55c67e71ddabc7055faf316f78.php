<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:36:"./template/pc/tfs/order\comment.html";i:1513818157;s:34:"./template/pc/tfs/user\header.html";i:1512197568;s:32:"./template/pc/tfs/user\menu.html";i:1519984571;s:36:"./template/pc/tfs/public\footer.html";i:1519984596;s:42:"./template/pc/tfs/public\sidebar_cart.html";i:1519984596;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
foreach($result as $key=>$v):       
	...
	<?php endif; ?>
			...
			<div class="col-md-10 my_right">
				...
				<div class="row">
					<div class="col-xs-12 set">
						<ul class="list-inline">
							<li>
								<a href="<?php echo U('/Home/Order/comment'); ?>" class="<?php if(\think\Request::instance()->param('status') != '0' AND \think\Request::instance()->param('status') != '1'): ?>current<?php endif; ?>">全部</a>
							</li>
							<li>
								<a href="<?php echo U('/Home/Order/comment',array('status'=>0)); ?>" class="<?php if(\think\Request::instance()->param('status') == '0'): ?>current<?php endif; ?>">待评论订单</a>
							</li>
							<li>
								<a href="<?php echo U('/Home/Order/comment',array('status'=>1)); ?>" class="<?php if(\think\Request::instance()->param('status') == '1'): ?>current<?php endif; ?>">已评论</a>
							</li>
						</ul>
					</div>
				</div>
				...
				<?php if(!empty($comment_list)): if(is_array($comment_list) || $comment_list instanceof \think\Collection || $comment_list instanceof \think\Paginator): $i = 0; $__LIST__ = $comment_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
						<div class="list">
							<div class="col-md-4">
								<dl>
									<dt><img src="<?php echo goods_thum_images($list['goods_id'],120,120); ?>" /></dt>
									<dd>
							...
							<?php if($list['is_comment'] == 1): ?>
								<div class="col-md-4 shou"><?php echo $list['content']; ?> </br>
									<span><?php echo date('Y-m-d H:i:s',$list['add_time']); ?></span></div>
								<?php else: ?>
								<div class="col-md-4 shou">您还没有对此商品进行评价哦~</br>     <span></span></div>
							<?php endif; ?>
							<div class="col-md-4 already">
								<?php if($list['is_comment'] == 1): ?>已评价
									<?php else: ?>
									未评价
								<?php endif; ?>
              <span>
                <?php if($list['is_comment'] == 1): ?>
	                ...
	                <?php else: ?>
	                <a href="<?php echo U('Home/Order/comment_list',array('order_id'=>$list['order_id'],'goods_id'=>$list['goods_id'])); ?>">评价</a>
                <?php endif; ?>
              </span>
							</div>
					...
					<div class="row list">
						<?php if(\think\Request::instance()->param('status') == '0'): ?>您还没有待评价订单，<a href="/">快去逛逛吧~</a>
							<?php elseif(\think\Request::instance()->param('status') == '1'): ?>您还没有已评价订单，<a href="<?php echo U('/Home/Order/comment',array('status'=>0)); ?>">快去评价吧~</a>
								<?php else: ?>
