<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:46:"./application/admin/view2/order\ajaxindex.html";i:1512439955;}*/ ?>
<table>
 	<tbody>
	...
	<?php else: if(is_array($orderList) || $orderList instanceof \think\Collection || $orderList instanceof \think\Paginator): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
  	<tr data-order-id="<?php echo $list['order_id']; ?>" id="<?php echo $list['order_id']; ?>">
        ...
        <td align="center" abbr="store_name" axis="col6" class="">
            <div style="text-align: center; width:120px;"class=""><?php echo (isset($list['store_name']) && ($list['store_name'] !== '')?$list['store_name']:'未分配'); ?></div>
        </td>
        <td align="center" abbr="safecode" axis="col6" class="">
            <div style="text-align: center; width:120px;" calss=""><?php echo (isset($list['safecode']) && ($list['safecode'] !== '')?$list['safecode']:'无'); ?></div>
        </td>
