<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:40:"./template/pc/rainbow/partner\apply.html";i:1512439866;s:38:"./template/pc/rainbow/user\header.html";i:1512439865;s:36:"./template/pc/rainbow/user\menu.html";i:1512439865;s:46:"./template/pc/rainbow/public\footer_index.html";i:1512439865;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>补货申请记录-<?php echo $tpshop_config['shop_info_store_title']; ?></title>
					...
					<div class="accbala">
						<table width="100%">
							...
							<tbody>
								...
								<tr>
									<td width="20%"><?php echo date("Y-m-d H:i:s",$vo['addtime']); ?></td>
									<td width="15%">
										<?php if($vo[status] == 0): ?>处理中<?php endif; if($vo[status] == 1): ?>已处理<?php endif; if($vo[status] == 2): ?>申请失败<?php endif; ?>
									</td>
									<td width="15%"><?php echo (isset($vo['express_name']) && ($vo['express_name'] !== '')?$vo['express_name']:"无"); ?></td>
									<td width="25%"><?php echo (isset($vo['express_code']) && ($vo['express_code'] !== '')?$vo['express_code']:"无"); ?></td>
									<td width="20%">
										<p><a href="<?php echo U('Partner/apply_info', array('id'=>$vo['id'])); ?>">查看详情</a></p>
										<?php if($vo['delivery_id']): ?>
											<p><a href="<?php echo U('Partner/invoiceInfo', array('id'=>$vo['delivery_id'])); ?>">发货单</a></p>
										<?php endif; ?>
									</td>
								</tr>
