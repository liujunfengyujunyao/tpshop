<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:47:"./template/mobile/new2/cart\ajax_cart_list.html";i:1512439835;}*/ ?>
            ...
            <div class="sc_list">
                <div class="radio fl ">
                    ...
                    <span onClick="checkgood(this)" class="che <?php if($v[selected] == 1): ?>check_t<?php endif; ?>" >
                        <i>
                            <input type="checkbox" autocomplete="off" id="good[<?php echo $v['id']; ?>]" name="cart_select[<?php echo $v['id']; ?>]" style="display:none;" value="1" onclick="ajax_cart_list();" <?php if($v[selected] == 1): ?>checked="checked"<?php endif; ?>>
                        </i>
                    </span>
                </div>
                    ...
                    <div class="prices">
                        ...
                        <?php if($v['store_count'] > 0): ?>
                            <div class="plus fr get_mp">
                                    ...
                                    <span class="mp_mp">
                                    <input id="goods_num[<?php echo $v['id']; ?>]" type="text" onKeyDown='if(event.keyCode == 13) event.returnValue = false' name="goods_num[<?php echo $v['id']; ?>]"  value="<?php echo $v['goods_num']; ?>"  class="input-num"  onblur="switch_num(0,<?php echo $v['id']; ?>,<?php echo $v['store_count']; ?>)" onkeyup="this.value=this.value.replace(/[^\d]/g,'')"/>
                                    </span>
                        ...
                        <?php else: ?>
                            <p class="sc_pri fr">
                            库存不足
                                <input id="goods_num[<?php echo $v['id']; ?>]" type="hidden" name="goods_num[<?php echo $v['id']; ?>]"  value="0"  class="input-num" />
                            </p>
                        <?php endif; ?>
                    </div>
                </div>
