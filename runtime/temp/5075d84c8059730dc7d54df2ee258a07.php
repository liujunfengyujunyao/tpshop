<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:46:"./template/pc/tfs/store\return_goods_list.html";i:1513818158;s:34:"./template/pc/tfs/user\header.html";i:1512197568;s:32:"./template/pc/tfs/user\menu.html";i:1512200314;s:36:"./template/pc/tfs/public\footer.html";i:1512208253;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta name="keywords" content="<?php echo $tpshop_config['shop_info_store_keyword']; ?>" />
	<meta name="description" content="<?php echo $tpshop_config['shop_info_store_desc']; ?>" />
	<title>退换货记录-<?php echo $tpshop_config['shop_info_store_title']; ?></title>
...
<div id="return_goods">
	<div class="container">
		<?php $pid =407;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1513818000 and end_time > 1513818000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
			...
			<div class="col-md-10 col-sm-9 col-lg-10">
				...
				<div class="my_right">
					<div class="per_main_r">
						<form action="<?php echo U('Home/Store/return_goods_list'); ?>" method="post">
							<ul class="good_back">
								<li>
									<label>退货状态:</label>
									<select class="form-control" name="status">
										<option value="0">全部状态</option>
										<option value="1" <?php if(\think\Request::instance()->param('status') == 1): ?>selected<?php endif; ?>>处理中</option>
										<option value="2" <?php if(\think\Request::instance()->param('status') == 2): ?>selected<?php endif; ?>>已完成</option>
										<option value="3" <?php if(\think\Request::instance()->param('status') == 3): ?>selected<?php endif; ?>>已取消</option>
									</select>
								</li>
								<li>
									<label>申请时间:</label>
						...
						<div class="stock_list_main">
							...
							<?php if(!empty($list)): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
									<ul class="stock_log_goods">
										<li><?php echo $v['order_sn']; ?></li>
										<li class="sto_goods_img">
											<div class="goods_img_pic"><a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$v[goods_id])); ?>"><img src="<?php echo goods_thum_images($v['goods_id'],80,80); ?>" width="80" height="80" /></a></div>
											<div class="goods_img_title"><h6><a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$v[goods_id])); ?>"><?php echo getSubstr($goodsList[$v[goods_id]],0,25); ?></a></h6></div>
										</li>
										<li class="width-sx"><?php echo date("Y-m-d",$v['addtime']); ?></li>
										<li class="width-sx"><?php echo $state[$v[status]]; ?></li>
										<li class="width-sx"><a href="<?php echo U('Home/Store/return_goods_info',array('id'=>$v[id])); ?>" class="view">查看详情</a></li>
									</ul>
					...
					<div class="eva_page"><?php echo $page; ?></div>
				</div>
			</div>
		</div>
	</div>
