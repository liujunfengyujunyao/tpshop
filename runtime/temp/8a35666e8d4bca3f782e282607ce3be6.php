<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:33:"./template/pc/tfs/cart\cart2.html";i:1512110325;s:41:"./template/pc/tfs/public\sign-header.html";i:1513580608;s:36:"./template/pc/tfs/public\footer.html";i:1519984596;s:42:"./template/pc/tfs/public\sidebar_cart.html";i:1519984596;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div id="order">
    ...
    <form name="cart2_form" id="cart2_form" method="post">
        ...
        <div class="container merch">
            ...
            <?php if(is_array($cartList) || $cartList instanceof \think\Collection || $cartList instanceof \think\Paginator): if( count($cartList)==0 ) : echo "" ;else: foreach($cartList as $k=>$v2): ?>
                <div class="row list">
                    <div class="col-xs-4">
                        <dl>
                            <dt><img src="<?php echo goods_thum_images($v2['goods_id'],80,80); ?>" width="100%"/></dt>
                            <dd><?php echo $v2['goods_name']; ?></dd>
                        </dl>
                    </div>
                    <div class="col-xs-2 tai">¥<?php echo $v2['goods_price']; ?></div>
                    <div class="col-xs-3 tai"><?php echo $v2['goods_num']; ?></div>
                    <div class="col-xs-3 tai">￥<?php echo $v2['goods_fee']; ?></div>
                </div>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            ...
            <div class="row excellent">
                 <div class="delivery">
                     使用优惠券 <span>（优惠券每次只能使用一张，不可多张混合使用）</span>
                 </div>
                <div class="row vouchers">
                    <div class="col-xs-12">
                        优惠券：&nbsp;<input type="radio" onclick="ajax_order_price();" value="1" checked="" name="couponTypeSelect" />&nbsp;&nbsp;
                        <select class="myselect" onchange="ajax_order_price();" id="coupon_id" name="coupon_id">
                        ...
                        <input type="radio" onclick="ajax_order_price();" value="2" name="couponTypeSelect" class="two">&nbsp;&nbsp;
                        <input type="text" placeholder="请输入优惠券代码" name="couponCode"/>&nbsp;&nbsp;
                        <a href="javascript:void(0)" onclick="ajax_order_price();">使 用</a>
                    </div>
                </div>
                ...
                <!--<div class="col-xs-12 two">余额支付：<input type="text" id="user_money" name="user_money" placeholder="请输入使用余额" onkeyup="this.value=/^\d+\.?\d{0,2}$/.test(this.value) ? this.value : ''" />&nbsp;&nbsp;<a href="javascript:void(0)" onClick="wield(this);">使用</a>&nbsp;&nbsp; 您的可用余额 ¥ <?php echo $user['user_money']; ?>-->
                <!--</div>-->
                <div class="delivery">其他信息</div>
                <div class="col-xs-12 vouchers">积分支付：<input type="text" id="pay_points" name="pay_points" placeholder="请输入使用积分" />&nbsp;&nbsp;<a href="javascript:void(0)" onClick="wield(this);">使 用</a>
                ...
                <div class="col-xs-12 vouchers">支付密码：<input type="password" id="paypwd" name="paypwd" placeholder="请输入支付密码" />&nbsp;&nbsp;<?php if(empty($user['paypwd'])): ?>请先<a href="<?php echo U('User/paypwd'); ?>" class="cipher">设置支付密码</a><?php endif; ?>
                </div>
        ...
        <div class="container checks">
            <div class="row mention">
                <p>商品总金额：<span id="order-cost-area">￥<?php echo $total_price['total_fee']; ?></span></p>
                <p>运费：<span>￥</span><span id="postFee">0</span></p>
                <p>使用优惠券：<span>-￥</span><span id="couponFee">0</span></p>
                <p>使用积分：<span>-￥</span><span id="pointsFee">0</span></p>
                <p>优惠活动：<span>-￥</span><span id="order_prom_amount">0</span></p>
                <!--<p>使用余额：<span>-￥</span><span id="balance">0</span></p>-->
