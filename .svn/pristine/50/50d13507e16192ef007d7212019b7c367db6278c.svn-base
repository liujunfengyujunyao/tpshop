...
class Distribut extends Base {
    ...
    public function tree(){                
        $where = 'us.is_distribut = 1 and us.first_leader = 0 and p.status=1';
        if($this->request->param('user_id'))
            $where = "ua.user_id = '{$this->request->param('user_id')}'";
        
        $list = M('users')->alias('us')->field('us.user_id,us.nickname,us.mobile,us.email,p.phone')->join('__PARTNER__ p','p.user_id = us.user_id')->where($where)->select();
    ...
    public function distributor_list(){
    	...
    	foreach ($user_list as $k=>$val){
    		...
    		// $user_list[$k]['third_leader'] = M('users')->where(array('third_leader'=>$val['user_id']))->count();目前没有三级
