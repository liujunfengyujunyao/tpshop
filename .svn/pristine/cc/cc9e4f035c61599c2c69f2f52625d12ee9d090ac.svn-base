...
class Goods extends Base {
    ...
    */ 
    public function goodsInfo(){
        ...
        //查询赠品信息    @author;Ly  @date:2017-9-21
        if(!empty($goods['send_id'])){
            $send_goods_info = M('Goods')->field('goods_id,goods_name')->where("goods_id",$goods['send_id'])->find();
            $this->assign('send_goods_info',$send_goods_info);
        }
...
//        $replyList = M('Comment')->where(['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>['>',0]])->order("add_time desc")->select();
    ...
    public function integralMall()
    {
        ...
        $goods_category = M('goods_category')->where(array('level' => 1, 'is_show'=>1))->select();
