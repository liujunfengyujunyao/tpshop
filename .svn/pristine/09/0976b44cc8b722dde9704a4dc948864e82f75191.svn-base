...
class Partner extends Model {
	...
	public function getPartnerStock($partner_id, $Page) {
		...
		$list = DB::name('partner_stock') -> alias('ps')
				-> field("ps.goods_id, g.goods_name, ps.stock_num, ps.goods_num, FROM_UNIXTIME(ps.edittime, '%Y-%m-%d %H:%i:%s') as edittime")
		...
		$partner = DB::name('partner_stock')->alias('ps')
				->field("ps.goods_id, ps.partner_id, ps.goods_num, FROM_UNIXTIME(ps.edittime, '%Y-%m-%d %H:%i:%s') as edittime, ps.stock_num, g.goods_name ,g.goods_sn")
				->join('goods g', 'g.goods_id = ps.goods_id', 'LEFT')
				->where("ps.partner_id=$partner_id")
				->order('ps.edittime desc, ps.goods_id')
				->limit($Page->firstRow . ',' . $Page->listRows)
	 ...
	 * @return array 库存列表
	 */
	public function store_stock_list($store_id, $pagesize=10) {
	 ...
	 * @return array 补货申请列表
	 */
	public function apply($where, $pagesize=16) {
	 ...
	 */
	public function goodsReport($where, $pagesize=10) {
		$count = Db::name('delivery')
				->alias('d')
