...
class Order extends Base {
    ...
    public function order_list(){
        ...
        //$order_list = M('order')->order($order_str)->where($where)->bind($bind)->limit(5)->select();
        //获取订单商品
    ...
    public function order_detail(){
        ...
        //获取工厂店 by Dh 2017-9-18
        if($order_info['shop_way'] == 2 || $order_info['shop_way'] == 3 ){
            $order_info['store_name'] = M('store')->where('store_id',$order_info['store_id'])->getfield('store_name');
        }
        //获取订单操作记录
    ...
    public function cancel_order_info(){
    	$order_id = I('order_id/d',0);
    	$order = M('order')->where(array('order_id'=>$order_id,'order_status'=>3))->find();
        //添加取消订单进度判断 by Dh 2017-10-17
        if (($order['order_status'] == 3) && ($order['pay_status'] == 1)) {
            $order['cancel_status'] = 2;
        }
        if (($order['order_status'] == 3) && ($order['pay_status'] == 3 || $order['pay_status'] == 4)) {
