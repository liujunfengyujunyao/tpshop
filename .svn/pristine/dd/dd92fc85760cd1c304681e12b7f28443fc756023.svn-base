...
// 随机字符串
  private function createNonceStr($length = 16) {
     ...
     * @date: 2017-9-5
     */
    public function get_qrcode($qrcode){
        $access_token = $this->get_access_token();
        $url = "https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token={$access_token}";  //获取ticket ＵＲＬ
        $return = httpRequest($url,'POST',$qrcode);
        $return = json_decode($return, true);
        $url_image = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" .urlencode($return['ticket']);
        return $url_image;
    }
