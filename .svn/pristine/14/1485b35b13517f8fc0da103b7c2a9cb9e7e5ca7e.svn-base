...
class IntegralLogic extends Model
{
    ...
    public function addOrder($invoice_title,$user_note,$shop_way,$store_id){
        /**
         * 查询用户推荐工厂店ID
         * @author: Ly
         * @date: 2017-8-23
         */
        $store_default = M('store')->alias('s')->join('__USERS__ u','u.first_leader = s.user_id')->where('u.user_id',$this->user['user_id'])->getfield('s.store_id');
        ...
        $orderData = [
            ...
            'shipping_code'    =>$this->shippingCode,//'物流编号',
        ...
        $orderGoodsData = [
            ...
            'give_integral'  => ($store_default == $store_id)? $this->goods['give_integral'] : 0, //购买商品赠送积分
        ];
        if(empty($this->specGoodsPrice)){
        ...
        if ($this->structure['order_amount'] == 0) {
            $ext = ['act'=>'integral'];
            update_pay_status($order_sn, $ext); // 这里刚刚下的订单必须从主库里面去查
        }
