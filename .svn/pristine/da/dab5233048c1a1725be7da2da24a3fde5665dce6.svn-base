...
class System extends Base
{
	...
	public function index()
	{
		...
		//如果为签到设置，获取会员等级列表 by DH 2017-12-27
		if ($inc_type == 'signin') {
			$level = M('User_level')->field('level_id, level_name, sign_way, sign_count, sign_integral')->where('is_special=0')->select();
	...
	public function signHandle() {
		...
		foreach ($param as $key=>$value) {
			DB::name('User_level')->where('level_id', $key)->update($value);
		}
		$this->success("操作成功",U('System/index',array('inc_type'=>$inc_type)));
	}
     ...
     */        
     public function login_task()
     {
        ...
        // 多少天后自动分销记录自动分成
		 ...
		 //更新合伙人最大库存 by Dh 2017-12-30
		 $stock = DB::name('partner')->alias('p')
				 ->field('ps.stock_id, p.partner_id, ps.goods_id, ps.stock_num')
				 ->join('partner_stock ps', 'ps.partner_id = p.partner_id', 'left')
				 ->where('p.status', 1)->select();
		 $partner = D('Partner');
		 foreach($stock as $key=>$value) {
			 $num = $partner->max_stock($value['partner_id'], $value['goods_id']);
			 if ($num > $value['stock_num']) {
				 DB::name('partner_stock')->where('stock_id', $value['stock_id'])->setField('stock_num', $num);
			 }
		 }
     }
