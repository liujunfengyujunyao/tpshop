...
class Index extends Base {
    ...
    public function index(){
        ...
        //获取促销活动列表 by DH 2017-11-4
        $where['is_close'] = 0;
        $where['start_time'] = array('lt', time());
        $where['end_time'] = array('gt', time());
        $promotion = M('Prom_goods')->field('name')->where($where)->select();
        ...
        $new_goods = M('Goods')->alias('g')
                    ...
                    ->order('g.sort, g.goods_id')->limit(6)->select();
        $this->assign('new_goods', $new_goods);

        //获取子分类 by DH 2017-11-4
        $cate_child1 = M('Goods_category')->field('id, name')->where(array('level'=>2, 'is_show'=>1, 'is_hot'=>1))->order('sort_order')->select();
        $this->assign('cate_child1', $cate_child1);

        //获取推荐商品 by DH 2017-11-6
        $recommend_goods = M('Goods')->alias('g')
                            ->join('goods_category c', 'c.id = g.cat_id', 'left')
                            ->field('g.goods_id, g.goods_name, g.shop_price, g.market_price')
                            ->where(array('g.is_recommend'=>1, 'g.is_on_sale'=>1, 'c.is_show'=>1))
                            ->order('g.sort, g.goods_id')->limit(9)->select();
        $this->assign('recommend_goods', $recommend_goods);

        //获取热销商品 by DH 2017-11-6
        $hot_goods = M('Goods')->alias('g')
                    ->join('goods_category c', 'c.id = g.cat_id', 'left')
                    ->field('g.goods_id, g.goods_name, g.shop_price, g.market_price')
                    ->where(array('g.is_hot'=>1, 'g.is_on_sale'=>1, 'c.is_show'=>1))
                    ->order('g.sort, g.goods_id')->limit(9)->select();
...
//        $this->assign('cateList',$cateList);
    ...
    function truncate_tables (){
        ...
        $table = array('tfs_admin','tfs_config','tfs_region','tfs_system_module','tfs_admin_role','tfs_system_menu','tfs_article_cat','tfs_wx_user');
        foreach($tables as $key => $val)
        {                                    
            if(!in_array($val['Tables_in_tfs'], $table))                             
                echo "truncate table ".$val['Tables_in_tfs'].' ; ';
                echo "<br/>";         
        }                
    }
    ...
    public function ajax_favorite(){
        ...
        //获取用户浏览记录 by Dh 2017-9-27
        $user = session('user');
        $favourite_goods = M('goods_visit a')
            ->field('a.goods_id, g.goods_name, g.shop_price, g.market_price')
            ->join('__GOODS__ g', 'a.goods_id = g.goods_id', 'LEFT')
            ->where(array('a.user_id'=>$user['user_id'], 'g.is_on_sale'=>1))
            ->order('a.visittime desc, g.goods_id desc')
            ->page($p,$i)
            ->cache(true,TPSHOP_CACHE_TIME)
            ->select();
        if (!$favourite_goods) {
            $favourite_goods = M('goods')->field('goods_id, goods_name, shop_price, market_price')->where("is_hot=1 and is_on_sale=1")->order('goods_id DESC')->page($p,$i)->cache(true,TPSHOP_CACHE_TIME)->select();
        }
