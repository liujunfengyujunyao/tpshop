...
class SmsLogic 
{
    ...
    public function sendSms($scene, $sender, $params, $unique_id=0)
    {
        ...
        $smsParams = array(
            ...
            4 => "{\"order_sn\":\"$order_sn\", \"datetime\":\"$datetime\"}",                                                                                               //4. 客户支付
            5 => "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\"}",  //5.商家发货
            6 => "{\"code\":\"$code\"}",                                                                                                            //6. 修改手机号码 (验证码类型短信只能有一个变量)
            7 => "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\",\"store_name\":\"$store_name\",\"store_user\":\"$store_user\",\"phone\":\"$phone\"}", //商家发货（工厂店配送）
            8 => "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\",\"address\":\"$address\",\"safe\":\"$safecode\"}", //商家发货（工厂店自提）
        );
