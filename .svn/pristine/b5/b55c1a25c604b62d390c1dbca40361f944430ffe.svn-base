...
class UsersLogic extends Model
{
    ...
    public function oauth_bind_new($user = array()){
        ...
        $thirdOauth = $_SESSION['third_oauth'];
        ...
        if($unionid){ 
            ...
            if($thirdUser && $thirdUser['user_id'] ){
            ...
            if($thirdUser && $thirdUser['is_band'] = 1){  			
        ...
        //4.账号绑定
		if($thirdOauth['unionid']){
			M('OauthUsers')->where(array('oauth'=>$oauth ,'unionid'=>$unionid))->save(array('user_id'=>$ruser['user_id'], 'is_bind'=>1, 'bind_time'=>time()));
		}else{
			M('OauthUsers')->where(array('oauth'=>$oauth, 'openid'=>$openid))->save(array('user_id'=>$ruser['user_id'], 'is_bind'=>1, 'bind_time'=>time()));
		}
		
		$oauth_user = M('OauthUsers')->where(array('oauth'=>$oauth ,'unionid'=>$unionid ,'user_id'=>$ruser['user_id']))->find();

		if($oauth_user['sark_code']){
			$sark_code = $oauth_user['sark_code'];

			$store_id = M('Sark')->where(array('sark_code'=>$sark_code))->getField('store_id');
			$store_info = M('Store')->where(array('store_id'=>$store_id))->field('user_id,partner_id')->find();

            if ($store_info) {
				$first_leader = $store_info['user_id']; // 第一级推荐人
				$second_leader = M('Partner')->where(array('partner_id'=>$store_info['partner_id']))->getField('user_id');
                $user['first_leader'] = $first_leader; //  第一级推荐人
                $user['second_leader'] = $second_leader; // 第二级推荐人
				M('Users')->where(array('user_id'=>$oauth_user['user_id']))->save($user);
                //他上线分销的下线人数要加1
    ...
    public function thirdLogin($data=array()){
        ...
        //获取用户信息
        if($data['unionid']){
        	$thirdUser = get_user_info($data['unionid'],4,$oauth);
        }else{
        	$thirdUser = get_user_info($openid,3,$oauth);
        }
        ...
        if(!$thirdUser){
            ...
            if ($data['sarkcode']) {
                $oauth_user['sark_code'] = $data['sarkcode'];
			}

			$ret = M('oauth_users')->insert($oauth_user);
			if($ret){
				$user = M('oauth_users')->where(array('openid'=>$openid))->find();
			}
				/*
                $store_id = M('Sark')->where(array('sark_code'=>$sark_code))->getField('store_id');
                $store_info = M('Store')->where(array('store_id'=>$store_id))->field('user_id,partner_id')->find();
                ...
                if ($store_info) {
                    $first_leader = $store_info['user_id']; // 第一级推荐人
                    $second_leader = M('Partner')->where(array('partner_id'=>$store_info['partner_id']))->getField('user_id');
                    $map['first_leader'] = $first_leader; //  第一级推荐人
                    $map['second_leader'] = $second_leader; // 第二级推荐人
                    //他上线分销的下线人数要加1
                    M('users')->where('user_id', $map['first_leader'])->setInc('underling_number');
                    M('users')->where('user_id', $map['second_leader'])->setInc('underling_number');
                    //M('users')->where('user_id', $map['third_leader'])->setInc('underling_number');
                }
            } else {
                $map['first_leader'] = 0;
            }     
			*/
			...
			$user = M('oauth_users')->where(array('unionid'=>$unionid))->find();
        }
    ...
    public function reg($username,$password,$password2, $push_id=0,$invite=array()){
        ...
        if($is_validated != 1)
            return array('status'=>-1,'msg'=>'请使用手机号注册');
    ...
    public function get_account_log($user_id,$account_type = 0){
        ...
        if($account_type == 0){
            ...
            $account_log_where['pay_points'] = ['eq',0];
        }
        if($account_type == 1){
     ...
     * @return mixed
     */
    public function get_points_log($user_id){
        $account_log_where = ['user_id'=>$user_id];
    ...
    public function get_goods_collect($user_id){
        ...
        //获取我的收藏列表
            ...
            ->field('c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.is_on_sale,g.store_count,g.cat_id,g.goods_sn ')
            ->join('goods g','g.goods_id = c.goods_id','INNER')
     ...
     */
    public function password($user_id,$new_password,$confirm_password,$is_update=true){
        $user = M('users')->where('user_id', $user_id)->find();
